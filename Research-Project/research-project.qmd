...
- **Feb 6**: Finalize the Team
- **Feb 25**: Interim Submission I
- **March 18**: Research Project Feedback Meeting
- **April 8**: Interim Submission II
- **May 6**: Final Submission
...
- **Control Variables**: These are variables that affect <u>both</u> the dependent and the independent variable, potentially confounding our primary relationship of interest. For example, 'month of sale' would be a confounding variable as it affects both sales and weather conditions. For instance, people tend to buy more gifts during the holiday season when the weather is also colder. So we might observe higher sales during colder months, but this could be due to the holiday season rather than the weather. To account for such confounding effects, you will incorporate the control variables you select later in your analysis.
...
flowchart LR
    X(Primary Independent Variable) --> Y(Dependent Variable)
    C(Control Variables) --> X & Y
```
