Tue 01/21 & 1 & \multirow{5}{=}{Describing Data} & Introductions; Summation notation &  \\
\cline{1-2} \cline{4-5}
Thu 01/23 & 2 &  & Distribution, mean, median, percentiles &  \\
\cline{1-2} \cline{4-5}
Tue 01/28 & 3 &  & Variance, standard deviation, Z-score &  \\
\cline{1-2} \cline{4-5}
Thu 01/30 & 4 &  & Covariance and correlation &  \\
\cline{1-2} \cline{4-5}
Tue 02/04 & 5 &  & Research questions and data & Problem Set 1 \\
\Xhline{2.2\arrayrulewidth}
Thu 02/06 & 6 & \multirow{3}{=}{Coding in R} & Getting started with R & RP Team \\
\cline{1-2} \cline{4-5}
Tue 02/11 & 7 &  & Importing and cleaning data in R &  \\
\cline{1-2} \cline{4-5}
Thu 02/13 & 8 &  & Describing variables in R &  \\
\Xhline{2.2\arrayrulewidth}
Tue 02/18 & 9 & \multirow{3}{=}{Random Variables} & Distribution, expectation, variance & Problem Set 2 \\
\cline{1-2} \cline{4-5}
Thu 02/20 & 10 &  & Normal distribution, Z-score &  \\
\cline{1-2} \cline{4-5}
Tue 02/25 & 11 &  & Independence, correlation & RP Submission 1 \\
\Xhline{2.2\arrayrulewidth}
Thu 02/27 & 12 & \multirow{3}{=}{Sampling and Estimation} & Sample mean distribution; Good estimators &  \\
\cline{1-2} \cline{4-5}
Tue 03/04 & 13 &  & Confidence intervals &  \\
\cline{1-2} \cline{4-5}
Thu 03/06 & 14 &  & Hypothesis testing and p-values & Problem Set 3 \\
\Xhline{2.2\arrayrulewidth}
Tue 03/11 & \multicolumn{3}{l}{Review Class} &  \\
\hline
Thu 03/13 & \multicolumn{3}{l}{Midterm Exam} &  \\
\hline
Tue 03/18 & \multicolumn{3}{l}{Research Project Feedback} &  \\
\Xhline{2.2\arrayrulewidth}
Thu 03/20 & 15 & \multirow{3}{=}{Linear Regression} & Ordinary least squares (OLS), Goodness of fit: $R^2$ &  \\
\cline{1-2} \cline{4-5}
Tue 03/25 & 16 &  & Prediction vs. causal inference &  \\
\cline{1-2} \cline{4-5}
Thu 03/27 & 17 &  & Inference (p-values, t-stats, confidence intervals) &  \\
\Xhline{2.2\arrayrulewidth}
 & \multicolumn{3}{l}{Spring Recess} &  \\
\Xhline{2.2\arrayrulewidth}
Tue 04/08 & 18 & \multirow{6}{=}{Linear Regression (cont.)} & Omitted variable bias; Multiple regression model; Adjusted $R^2$ & RP Submission 2 \\
\cline{1-2} \cline{4-5}
Thu 04/10 & 19 &  & Categorical variables; Interaction terms &  \\
\cline{1-2} \cline{4-5}
Tue 04/15 & 20 &  & Quadratic and log functional forms &  \\
\cline{1-2} \cline{4-5}
Thu 04/17 & 21 &  & Recap and synthesis &  \\
\cline{1-2} \cline{4-5}
Tue 04/22 & 22 &  & Linear regression in R & Problem Set 4 \\
\cline{1-2} \cline{4-5}
Thu 04/24 & 23 &  & Linear regression in R &  \\
\Xhline{2.2\arrayrulewidth}
Tue 04/29 & 24 & \multirow{3}{=}{Advanced Topics} & Experiments and quasi-experimental methods &  \\
\cline{1-2} \cline{4-5}
Thu 05/01 & 25 &  & Panel data and event study designs &  \\
\cline{1-2} \cline{4-5}
Tue 05/06 & 26 &  & Big data and machine learning & Final Paper \\
\Xhline{2.2\arrayrulewidth}
Thu 05/08 & \multicolumn{3}{l}{Review Class} &  \\
\hline
Thu 05/15 & \multicolumn{3}{l}{Final Exam (1--2.50 pm)} &  \\
\hline
