...
%\vskip-0.5em
%\begin{witemize}
%  \item Say, $X$ is a random variable with mean $\mu_X$, variance $\sigma^2_X$, and standard deviation $\sigma_X$
%  \item If we transform $X$ to create a new random variable $$ Y = a + bX $$
%\item Then $Y$ is a random variable that has a distribution with the same shape as $X$ and with \vskip0.15em
%\begin{mitemize}
%\normalsize
%  \item Mean: $\mu_Y = a + b \cdot \mu_X \quad  $
%  \item Variance: $\sigma^2_Y = b^2 \cdot \sigma^2_X$
%  \item SD:  $\sigma_Y = b \cdot \sigma_X  $
%\end{mitemize}
%\end{witemize}
%\end{frame}
%
...
\draw[dotted, line width=1pt](axis cs: 3, 0)--(axis cs: 3, 0.1);
\end{axis}
\end{tikzpicture}
 \end{figure}
\end{frame}
...
\draw[dotted, line width=1pt](axis cs: 0, 0)--(axis cs: 0, 0.4);
\end{axis}
\end{tikzpicture}
 \end{figure}
\end{frame}
...
\vskip-0.5em
For example, say $X \sim N(3, 16)$ and we are given $ Pr(X>x) = 0.10$. How to find $x$? \vskip0.5em
\begin{figure}[!h]
 \centering	
\begin{tikzpicture}
\begin{axis}[
  ...
  %every axis y label/.style={at=(current axis.above origin),anchor=south},
  height=6cm, width=10cm,
  xtick={3, 8.12}, %ytick=\empty,
  xticklabels ={3, x?},
  enlargelimits=false, clip=false, axis on top,
  ]
  \addplot [very thick,black] {gauss(3, 4)}; 
  \addplot [fill=purple!20, draw=none, domain=8.12:15] {gauss(3, 4)} \closedcycle;
\draw[dotted, line width=1pt](axis cs: 3, 0)--(axis cs: 3, 0.1);
\node at (axis cs: 10, 0.01) {$0.1$};
\end{axis}
\end{tikzpicture}
 \end{figure}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Finding points from area under the curve}
Start by finding $z$, such that $Pr(Z>z)=0.1$. \vskip0.5em
\begin{figure}[!h]
 \centering	
\begin{tikzpicture}
\begin{axis}[
  no markers, domain=3-3*4:3+3*4, samples=100,
  axis lines*=left, xlabel=, ylabel=$f(Z)$,
  %every axis y label/.style={at=(current axis.above origin),anchor=south},
  height=6cm, width=10cm,
  xtick={3, 8.12}, %ytick=\empty,
  xticklabels ={0, 1.28},
...
%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Finding points from area under the curve}
Transforming $z$ back to $x = 3+1.28 \times 4 = 8.12$.  \vskip0.5em
\begin{figure}[!h]
 \centering	
\begin{tikzpicture}
\begin{axis}[
